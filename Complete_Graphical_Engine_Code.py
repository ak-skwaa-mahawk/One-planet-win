...
if not self.active_incident.startswith("SUCCESS"):self.active_incident = "Calculations cycle synchronized neutrally."self.turn += 1self.update_ui_display()self.check_game_over()def update_ui_display(self):# Update General Panels text parametersself.lbl_turn.config(text=f"CYCLE: {self.turn:03d}")self.lbl_integrity.config(text=f"GLOBAL MATRIX LOCK: {self.tether_integrity}%")self.lbl_hype.config(text=f"ADVERSARIAL HYPER-STATIC: {self.global_hype*100:.1f}%")self.lbl_energy.config(text=f"ENERGY: {int(self.energy_pool)}/{int(self.max_energy)} MJ")self.incident_box.config(text=f"LOG INTERACTION: {self.active_incident}")# Update each Sector Progressbar and metric textsfor sector_id, ui_obj in self.ui_sectors.items():data = self.grid[sector_id]current_logic = self.compute_noisy_logic(sector_id)ui_obj["pbar"]['value'] = data["tether"]buff_txt = f"SHIELD({data['shield']})" if data["shield"] > 0 else "NONE"ui_obj["metrics"].config(text=f" | Logic: {current_logic:5.1f}% | Drag: {data['drag']:4.1f} | Buff: {buff_txt:<9}")def check_game_over(self):if self.tether_integrity >= 90.0 and self.global_hype <= 0.20:messagebox.showinfo("VICTORY MATRIX LOCKED", f"🏆 SUCCESS!\nAll nodes fully locked to the universal Tether using absolute baseline logic by cycle {self.turn}!")self.root.quit()elif self.turn > 50 and self.tether_integrity < 50.0:messagebox.showerror("MATRIX FAILURE", "❌ CONNECTION LOST!\nSystemic friction and ideological hype decoupled the Earth-to-Cosmos grid bridge permanently.")self.root.quit()if name == "main":root = tk.Tk()app = HardenedCosmicGUI(root)root.mainloop()